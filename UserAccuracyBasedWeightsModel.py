...
#query = "select id, name, created_at, settled_at, settled_value  from questions where settled_value is not null and type = 'binary' and (created_at between '2013-10-30' AND '2015-02-28') and (settled_at between '2013-10-30' AND '2015-02-28')"
query = "select id, name, created_at, settled_at, settled_value  from questions where settled_value is not null and type = 'binary' and (created_at between '2015-03-01' AND '2015-06-30') and (settled_at between '2015-03-01' AND '2015-06-30')"
...
query = "select distinct user_id from historical_trades where question_id in (select id from questions where type = 'binary' and settled_value is not null and (created_at between '2015-03-01' AND '2015-06-30') and (settled_at between '2015-03-01' AND '2015-06-30'))"
...
lookupdf = pd.DataFrame(0.000196425, index = binarytraders, columns = categories['name'])
...
for uid in binarytraders:
    ...
    if(len(trades_user) > 0): #Need to check again 
        ...
        for c1 in range(0, len(cat1)):
             lookupdf[cat1.index[c1]][uid] = 1-cat1.values[c1]
        for c2 in range(0, len(cat2)):
             lookupdf[cat2.index[c2]][uid]= 1-cat2.values[c2]
...
users.plot(kind = 'barh', legend = False, figsize = (8,8),fontsize=12)
...
usersBigdata.to_csv("D:\\Fall 2019\\CAP\\weighted avg\\usersBigdata_sub2.csv")         
lookupdf.to_csv("D:\\Fall 2019\\CAP\\weighted avg\\lookup_Sub2.csv")

############Plot errors
past = pd.read_csv("D:\\Fall 2019\\CAP\\weighted avg\\lookup_Sub.csv")
past.index = past['User']
past.drop(['User'], axis =1, inplace = True)
for trader in binarytraders:
    rec1 = past[past.index == trader]
    rec2 = lookupdf[lookupdf.index == trader]   
    rec3 = rec1-rec2
    j2 = [i for i in rec3 if rec3.values > 0]
    j2.plot(kind = 'bar')
    #rec2.plot(kind = 'bar')
